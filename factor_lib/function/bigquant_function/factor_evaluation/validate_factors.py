...
验证“因子发现 -> 数据适配 -> 因子计算 -> 输出结构”的完整调用链。
本版本对齐分粒度 FactorDataBundle loader：适配器上下文由 FACTOR 参数与
ADAPTER_SPEC.context_parameters 自动解析，不再保留 adapter_options 接口。
"""
...
def _query_recent_trading_dates(end_date, required_count, max_history_days):
    ...
    required_total = required_count + max_history_days
    if len(dates) < required_total:
        raise ValueError(
            "交易日历不足以同时覆盖测试日期和最长因子预热窗口："
            f"需要至少 {required_total} 个交易日，实际只有 {len(dates)} 个。"
        )
    return dates[-required_count:], dates


def _build_raw_dates(calendar, target_dates, history_days):
    """为所有目标日构造包含各自预热区间的原始数据日期集合。"""
    ...
    for target_date in target_dates:
        ...
        if position is None or position < history_days:
            raise ValueError(
                f"目标日期 {target_date:%Y-%m-%d} 的预热数据不足 "
                f"{history_days} 个交易日。"
            )
...
def _output_factor_column(metadata):
    """从 FACTOR.output_schema 解析唯一因子数值列。"""
...
def _security_daily_row_count(raw_data):
    """兼容普通 DataFrame 与分粒度 FactorDataBundle。"""
...
def validate_factors(
    ...
    """批量验证指定因子能否在 BigQuant 完整调用链中正常计算。

    参数
    ----
    end_date : str 或 datetime
        测试的截止日。函数自动选择此前最近的目标交易日。
    factor_names : str 或 sequence[str]，可选
        待验证因子；None 表示动态发现的全部因子。
    factor_params_by_name : dict，可选
        ``{因子名: {参数名: 参数值}}``，覆盖指定因子的默认参数。
    instruments : sequence[str]，可选
        测试股票池；None 表示由适配器加载可用全 A 面板。
    target_date_count : int，默认 30
        连续验证的目标交易日数，最少为 30。
    keep_factor_results : bool，默认 False
        True 时返回成功因子的完整输出；默认关闭以节省内存。
    ...
    ----
    dict
        ``summary`` 为每因子一行汇总，``target_dates`` 为本次目标日；
        当 keep_factor_results=True 时额外包含 ``factor_results``。
    """
    if not isinstance(target_date_count, int) or isinstance(
        target_date_count, bool
    ):
    ...
    for name in names:
        ...
        history_days = int(
            requirements.get("data_window", {}).get(
    ...
    for position, name in enumerate(names, start=1):
        ...
        if show_progress:
            print(
                f"[因子运行验证] {position}/{len(names)} 开始：{name}"
                f"（{len(target_dates)} 个目标日）...",
                flush=True,
            )
        try:
            ...
            # 适配器专属上下文由当前 FACTOR 的参数和 ADAPTER_SPEC 自动解析。
            ...
            base_record["non_null_ratio"] = float(
                pd.to_numeric(
                    factor_data[factor_column], errors="coerce"
                ).notna().mean()
            )
    ...
    summary = pd.DataFrame(records).sort_values(
        ["status", "factor_name"], kind="mergesort"
    ...
    result = {"summary": summary, "target_dates": target_dates}
    if keep_factor_results:
