...
对于会明显耗时的循环，进度单位应与实际工作量对应。因子截面、证券或参数组合等高层循环可按 `progress_every` 节流；但机器学习训练必须每个 batch 刷新一次。若单个 batch 还包含多个耗时阶段，应在同一行展示当前子阶段，例如“生成样本 → 判别器更新 → 生成器更新”或“前向计算 → 反向传播 → 参数更新”。

单行进度统一包含：阶段、完成数/总数、百分比、当前日期或任务、已耗时和预计剩余时间。使用 `\r`、`end=""` 与 `flush=True` 覆盖上一行，正常完成或异常退出时必须补一个换行。进度只反映真实可观测的工作单元；对无法获得内部完成度的平台查询，只显示简洁的开始和完成状态，不伪造百分比。进度显示默认关闭，不得改变计算结果、点时口径或模型训练语义。
