...
OUTPUT_COLUMNS = ["date", "instrument", "operating_cashflow_to_parent_net_profit_q"]
REQUIRED_COLUMNS = {"date", "instrument", "quarterly_operating_cashflow_to_parent_net_profit"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("operating_cashflow_to_parent_net_profit_q 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_operating_cashflow_to_parent_net_profit_q(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    """计算 operating_cashflow_to_parent_net_profit_q。

    factor_t = quarterly_operating_cashflow_to_parent_net_profit_t。
    ...
    if missing_columns:
        raise ValueError("operating_cashflow_to_parent_net_profit_q 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "quarterly_operating_cashflow_to_parent_net_profit"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("operating_cashflow_to_parent_net_profit_q 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("operating_cashflow_to_parent_net_profit_q 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("operating_cashflow_to_parent_net_profit_q 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[operating_cashflow_to_parent_net_profit_q] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        values = pd.to_numeric(df["quarterly_operating_cashflow_to_parent_net_profit"], errors="coerce")
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "operating_cashflow_to_parent_net_profit_q": values.to_numpy(dtype=float),
        })
        ...
        if show_progress:
            ...
            print(
                "\r[operating_cashflow_to_parent_net_profit_q] 计算完成 | "
...
FACTOR = {
    "name": "operating_cashflow_to_parent_net_profit_q",
    "func": calc_operating_cashflow_to_parent_net_profit_q,
    "factor_type": "base",
    "category": "earnings_quality",
    "direction": 1,
    "description": "净利润现金含量（单季度）：经营现金流净额相对归母净利润。",
    "formula": "factor_t = quarterly_operating_cashflow_to_parent_net_profit_t。",
    "input_schema": {
        "required": {
            ...
            "quarterly_operating_cashflow_to_parent_net_profit": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的净利润现金含量（单季度）。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "operating_cashflow_to_parent_net_profit_q": {"dtype": "float64", "meaning": "净利润现金含量（单季度）：经营现金流净额相对归母净利润。"},
    },
    ...
    "tags": ["earnings_quality", "financial", "base_factor"],
    "status": "research",
    "version": "1.0.0",
}
...
直接使用单季度净利润现金含量。数值高通常表示归母利润有更强的经营现金流支撑；极端值需在研究层检验。
