...
OUTPUT_COLUMNS = ["date", "instrument", "gross_profit_margin_ttm"]
REQUIRED_COLUMNS = {"date", "instrument", "ttm_gross_profit_margin"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("gross_profit_margin_ttm 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_gross_profit_margin_ttm(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    """计算 gross_profit_margin_ttm。

    factor_t = ttm_gross_profit_margin_t。
    ...
    if missing_columns:
        raise ValueError("gross_profit_margin_ttm 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "ttm_gross_profit_margin"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("gross_profit_margin_ttm 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("gross_profit_margin_ttm 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("gross_profit_margin_ttm 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[gross_profit_margin_ttm] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        values = pd.to_numeric(df["ttm_gross_profit_margin"], errors="coerce")
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "gross_profit_margin_ttm": values.to_numpy(dtype=float),
        })
        ...
        if show_progress:
            ...
            print(
                "\r[gross_profit_margin_ttm] 计算完成 | "
...
FACTOR = {
    "name": "gross_profit_margin_ttm",
    "func": calc_gross_profit_margin_ttm,
    "factor_type": "base",
    "category": "quality",
    "direction": 1,
    "description": "销售毛利率（TTM）。",
    "formula": "factor_t = ttm_gross_profit_margin_t。",
    "input_schema": {
        "required": {
            ...
            "ttm_gross_profit_margin": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的销售毛利率（TTM）。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "gross_profit_margin_ttm": {"dtype": "float64", "meaning": "销售毛利率（TTM）。"},
    },
