...
OUTPUT_COLUMNS = ["date", "instrument", "total_assets_turnover_q"]
REQUIRED_COLUMNS = {"date", "instrument", "quarterly_total_assets_turnover"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("total_assets_turnover_q 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_total_assets_turnover_q(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    """计算 total_assets_turnover_q。

    factor_t = quarterly_total_assets_turnover_t。
    ...
    if missing_columns:
        raise ValueError("total_assets_turnover_q 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "quarterly_total_assets_turnover"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("total_assets_turnover_q 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("total_assets_turnover_q 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("total_assets_turnover_q 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[total_assets_turnover_q] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        values = pd.to_numeric(df["quarterly_total_assets_turnover"], errors="coerce")
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "total_assets_turnover_q": values.to_numpy(dtype=float),
        })
        ...
        if show_progress:
            ...
            print(
                "\r[total_assets_turnover_q] 计算完成 | "
...
FACTOR = {
    "name": "total_assets_turnover_q",
    "func": calc_total_assets_turnover_q,
    "factor_type": "base",
    "category": "operating",
    "direction": 1,
    "description": "总资产周转率（单季度）。",
    "formula": "factor_t = quarterly_total_assets_turnover_t。",
    "input_schema": {
        "required": {
            ...
            "quarterly_total_assets_turnover": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的总资产周转率（单季度）。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "total_assets_turnover_q": {"dtype": "float64", "meaning": "总资产周转率（单季度）。"},
    },
    ...
    "tags": ["operating", "financial", "base_factor"],
    "status": "research",
    "version": "1.0.0",
}
