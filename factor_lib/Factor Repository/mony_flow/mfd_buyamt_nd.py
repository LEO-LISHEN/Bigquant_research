...
FACTOR = {
    ...
    "direction": 1,
    "description": (
        "过去 N 个交易日主力流入额累计值的反向因子，经稳健标准化、"
        "流通市值和可选行业中性化，并对残差再次稳健标准化；"
        "最终数值越高，按旧华泰研究方向越优。"
    ),
    "formula": (
        "sum_inflow_t=Sum(main_inflow_amount_{t-N+1:t})；"
        "raw_t=-sum_inflow_t；raw逐截面执行scaled-MAD去极值和总体"
    ...
    "output_schema": {
        ...
        "mfd_buyamt_nd": {
            "dtype": "float64",
            "meaning": (
                "市值行业中性化后的N日累计主力流入额反向因子；"
                "数值越高，按旧华泰研究方向越优。"
            ),
        },
    },
    "usage_notes": [
        "n_days=1复现旧notebook的mfd_buyamt_d。",
    ...
    "best_practice": {
        ...
        "description": "当前最佳实践保留原研究的单日主力流入额实例。",
    },
    "pit_notes": [
    ...
    "references": [
        (
            ...
            "mfd_buyamt_d因子/mfd_buyamt_d.ipynb。"
        ),
