...
def _prepare_daily_panel(data, target_dates, as_of_date, required_columns):
    ...
    if missing: raise ValueError(f"low_relative_return 缺少输入字段：{sorted(missing)}")
...
OUTPUT_COLUMNS = ["date", "instrument", "low_relative_return"]


def _resolve_low_relative_return_data_window(_):
...
def calc_low_relative_return(data, target_dates=None, as_of_date=None, show_progress=False, progress_every=20):
    """计算 ``low_t / close_{t-1} - 1``。"""
    del progress_every
    started_at = time.perf_counter()
    df, targets = _prepare_daily_panel(data, target_dates, as_of_date, ("low", "close"))
    if df.empty or targets.empty:
        return pd.DataFrame(columns=OUTPUT_COLUMNS)
    if show_progress:
        print("\r[low_relative_return] [1/2] 计算前收相对最低价...", end="", flush=True)
    try:
        previous_close = df.groupby("instrument", sort=False)["close"].shift(1)
        valid = np.isfinite(df["low"]) & np.isfinite(previous_close) & (previous_close != 0)
        values = pd.Series(np.nan, index=df.index, dtype=float)
        values.loc[valid] = df.loc[valid, "low"] / previous_close.loc[valid] - 1.0
        result = df.loc[df["date"].isin(targets), ["date", "instrument"]].copy()
        result["low_relative_return"] = values.loc[result.index].to_numpy()
        if show_progress:
            print(f"\r[low_relative_return] [2/2] 完成 | 耗时 {time.perf_counter() - started_at:.1f}s", end="", flush=True)
...
FACTOR = {
    "name": "low_relative_return", "func": calc_low_relative_return,
    "input_schema": {"required": {"date": {}, "instrument": {}, "low": {}, "close": {}}, "conditional": {}},
    "parameters": {"target_dates": {"default": None}, "as_of_date": {"default": None}, "show_progress": {"default": False}, "progress_every": {"default": 20}},
    "data_window": {"resolver": _resolve_low_relative_return_data_window, "default": _resolve_low_relative_return_data_window({})},
    "output_schema": {"date": {}, "instrument": {}, "low_relative_return": {"dtype": "float64"}},
}

FACTOR_INFO = """# low_relative_return\n\n日内最大下行幅度，计算当日最低价相对上一交易日收盘价的变化：`low_t / close_{t-1} - 1`。数值越小，表示该日盘中下探越明显。"""
