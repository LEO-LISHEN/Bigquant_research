...
OUTPUT_COLUMNS = ["date", "instrument", "roe_avg_ttm"]
REQUIRED_COLUMNS = {"date", "instrument", "ttm_roe_avg"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("roe_avg_ttm 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_roe_avg_ttm(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    if missing_columns:
        raise ValueError("roe_avg_ttm 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "ttm_roe_avg"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("roe_avg_ttm 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("roe_avg_ttm 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("roe_avg_ttm 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[roe_avg_ttm] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        values = pd.to_numeric(df["ttm_roe_avg"], errors="coerce")
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "roe_avg_ttm": values.to_numpy(dtype=float),
        })
...
FACTOR = {
    "name": "roe_avg_ttm",
    "func": calc_roe_avg_ttm,
    "factor_type": "base",
    "category": "quality",
    "direction": 1,
    "description": "平均净资产收益率（TTM）。",
    "formula": "factor_t = ttm_roe_avg_t。",
    "input_schema": {
        "required": {
            ...
            "ttm_roe_avg": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的平均净资产收益率（TTM）。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "roe_avg_ttm": {"dtype": "float64", "meaning": "平均净资产收益率（TTM）。"},
    },
...
直接使用 TTM 平均 ROE；不与现有单季、规模行业中性化的 qfa_roe 重复。
