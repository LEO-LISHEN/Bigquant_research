# -*- coding: utf-8 -*-
"""净利润 TTM 同比增长率相对市盈率的成长估值因子（G/PE）。"""
...
OUTPUT_COLUMNS = ["date", "instrument", "growth_to_pe"]
REQUIRED_COLUMNS = {"date", "instrument", "ttm_net_profit_yoy", "pe_ttm"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("growth_to_pe 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_growth_to_pe(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    factor_t = ttm_net_profit_yoy_t / pe_ttm_t。
    ...
    if missing_columns:
        raise ValueError("growth_to_pe 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "ttm_net_profit_yoy", "pe_ttm"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("growth_to_pe 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("growth_to_pe 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("growth_to_pe 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[growth_to_pe] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        numerator = pd.to_numeric(df["ttm_net_profit_yoy"], errors="coerce")
        denominator = pd.to_numeric(df["pe_ttm"], errors="coerce")
        ...
        result = pd.DataFrame({
            ...
            "growth_to_pe": values.to_numpy(dtype=float),
        })
...
FACTOR = {
    "name": "growth_to_pe",
    "func": calc_growth_to_pe,
    "factor_type": "base",
    "category": "value_growth",
    "direction": 1,
    "description": "净利润 TTM 同比增长率相对市盈率的成长估值因子（G/PE）。",
    "formula": "factor_t = ttm_net_profit_yoy_t / pe_ttm_t。",
    "input_schema": {
        "required": {
            ...
            "ttm_net_profit_yoy": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的净利润同比增长率（TTM）。"},
            "pe_ttm": {"dtype": "float", "frequency": "daily", "meaning": "目标日滚动市盈率，必须为正。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "growth_to_pe": {"dtype": "float64", "meaning": "净利润 TTM 同比增长率相对市盈率的成长估值因子（G/PE）。"},
    },
    ...
    "tags": ["value_growth", "financial", "base_factor"],
    "status": "research",
    "version": "1.0.0",
}
...
以净利润 TTM 同比增长率除以正的 TTM 市盈率。负增长会保留负值，负或零 PE 不参与计算。
