...
OUTPUT_COLUMNS = ["date", "instrument", "roa_avg_q"]
REQUIRED_COLUMNS = {"date", "instrument", "quarterly_roa_avg"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("roa_avg_q 缺少目标日期的原始数据：" + str(preview))
    return target_index
...
def calc_roa_avg_q(
    ...
    if missing_columns:
        raise ValueError("roa_avg_q 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "quarterly_roa_avg"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("roa_avg_q 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("roa_avg_q 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("roa_avg_q 输入存在重复 date + instrument：" + str(examples))
    ...
    try:
        values = pd.to_numeric(df["quarterly_roa_avg"], errors="coerce")
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "roa_avg_q": values.to_numpy(dtype=float),
        })
...
FACTOR = {
    ...
    "category": "quality",
    "direction": 1,
    "description": "平均总资产净利率（单季度）。",
    "formula": "factor_t = quarterly_roa_avg_t。",
    "input_schema": {
        "required": {
            ...
            "quarterly_roa_avg": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的平均总资产净利率（单季度）。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "roa_avg_q": {"dtype": "float64", "meaning": "平均总资产净利率（单季度）。"},
    },
...
直接使用单季度平均 ROA，衡量公司以平均总资产创造净利润的效率。
