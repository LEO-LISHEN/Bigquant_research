...
def _prepare_daily_panel(data, target_dates, as_of_date, required_columns):
    ...
    if missing: raise ValueError(f"rsi_nd 缺少输入字段：{sorted(missing)}")
...
OUTPUT_COLUMNS = ["date", "instrument", "rsi_nd"]


def _resolve_rsi_nd_data_window(params):
    ...
    return {"lookback_trading_days": int(window), "requires_target_date_data": True, "minimum_history_observations": int(window), "preheating_required": True, "insufficient_window_behavior": "涨跌幅窗口不足时输出 NaN。"}


def calc_rsi_nd(data, target_dates=None, as_of_date=None, window=14, show_progress=False, progress_every=20):
    """按简单移动均值计算 ``RSI(N)``，取值通常在 0 至 100。"""
    del progress_every
    _resolve_rsi_nd_data_window({"window": window}); started_at = time.perf_counter()
    ...
    if show_progress: print(f"\r[rsi_nd] [1/2] 计算 {window} 日 RSI...", end="", flush=True)
    try:
        delta = df.groupby("instrument", sort=False)["close"].diff()
        gains, losses = delta.clip(lower=0), (-delta.clip(upper=0))
        average_gain = gains.groupby(df["instrument"], sort=False).transform(lambda item: item.rolling(int(window), min_periods=int(window)).mean())
        average_loss = losses.groupby(df["instrument"], sort=False).transform(lambda item: item.rolling(int(window), min_periods=int(window)).mean())
        values = pd.Series(np.nan, index=df.index, dtype=float)
        normal = np.isfinite(average_gain) & np.isfinite(average_loss) & (average_loss > 0)
        values.loc[normal] = 100.0 - 100.0 / (1.0 + average_gain.loc[normal] / average_loss.loc[normal])
        values.loc[(average_loss == 0) & (average_gain > 0)] = 100.0
        values.loc[(average_loss == 0) & (average_gain == 0)] = 50.0
        result = df.loc[df.date.isin(targets), ["date", "instrument"]].copy(); result["rsi_nd"] = values.loc[result.index].to_numpy()
        if show_progress: print(f"\r[rsi_nd] [2/2] 完成 | 耗时 {time.perf_counter()-started_at:.1f}s", end="", flush=True)
...
FACTOR = {"name": "rsi_nd", "func": calc_rsi_nd,
          ...
          "output_schema": {"date": {}, "instrument": {}, "rsi_nd": {"dtype": "float64"}}}
FACTOR_INFO = """# rsi_nd\n\n相对强弱指标。先计算 N 日内日收益的平均上涨幅度与平均下跌幅度，再计算 `RSI = 100 - 100 / (1 + 平均上涨 / 平均下跌)`。参数 `window` 默认 14，取值通常在 0 至 100 之间。"""
