...
def _prepare_daily_panel(data, target_dates, as_of_date, required_columns):
    ...
    if missing: raise ValueError(f"volume_relative_ma_nd 缺少输入字段：{sorted(missing)}")
...
OUTPUT_COLUMNS = ["date", "instrument", "volume_relative_ma_nd"]


def _resolve_volume_relative_ma_nd_data_window(params):
    window = params.get("window", 20)
    if not isinstance(window, (int, np.integer)) or isinstance(window, (bool, np.bool_)) or window < 1:
        raise ValueError("window 必须是正整数。")
    return {"lookback_trading_days": int(window) - 1, "requires_target_date_data": True,
            "minimum_history_observations": int(window) - 1, "preheating_required": window > 1,
            "insufficient_window_behavior": "滚动窗口不足或均量为零时输出 NaN。"}


def calc_volume_relative_ma_nd(data, target_dates=None, as_of_date=None, window=20, show_progress=False, progress_every=20):
    """计算 ``volume_t / MA(volume, window)_t - 1``。"""
    del progress_every
    config = _resolve_volume_relative_ma_nd_data_window({"window": window})
    ...
    if show_progress: print(f"\r[volume_relative_ma_nd] [1/2] 计算 {window} 日均量...", end="", flush=True)
    try:
        ...
        values = pd.Series(np.nan, index=df.index, dtype=float); values.loc[valid] = df.loc[valid, "volume"] / average.loc[valid] - 1.0
        result = df.loc[df.date.isin(targets), ["date", "instrument"]].copy(); result["volume_relative_ma_nd"] = values.loc[result.index].to_numpy()
        if show_progress: print(f"\r[volume_relative_ma_nd] [2/2] 完成 | 耗时 {time.perf_counter()-started_at:.1f}s", end="", flush=True)
        return result.sort_values(["date", "instrument"], kind="mergesort").reset_index(drop=True)
    finally:
        if show_progress: print()


FACTOR = {"name": "volume_relative_ma_nd", "func": calc_volume_relative_ma_nd,
          ...
          "data_window": {"resolver": _resolve_volume_relative_ma_nd_data_window, "default": _resolve_volume_relative_ma_nd_data_window({})},
          "output_schema": {"date": {}, "instrument": {}, "volume_relative_ma_nd": {"dtype": "float64"}}}
FACTOR_INFO = """# volume_relative_ma_nd\n\n量能偏离因子：`volume_t / MA(volume, N)_t - 1`，其中 `MA` 为包含目标日的简单移动均值。参数 `window` 控制回看交易日数，默认 20。"""
