# -*- coding: utf-8 -*-
"""扣非净利润相对市值的估值因子，数值越高代表扣非盈利相对价格越高。"""
...
OUTPUT_COLUMNS = ["date", "instrument", "earnings_to_price_deducted"]
REQUIRED_COLUMNS = {"date", "instrument", "ttm_deducted_net_profit", "total_market_cap"}
...
def _normalize_target_dates(target_dates, available_dates):
    ...
    if not missing.empty:
        ...
        raise ValueError("earnings_to_price_deducted 缺少目标日期的原始数据：" + str(preview))
    return target_index


def calc_earnings_to_price_deducted(
    data,
    target_dates=None,
    as_of_date=None,
    show_progress=False,
    ...
    """计算 earnings_to_price_deducted。

    factor_t = ttm_deducted_net_profit_t / total_market_cap_t。
    ...
    if missing_columns:
        raise ValueError("earnings_to_price_deducted 因子缺少字段：" + str(sorted(missing_columns)))

    df = data.loc[:, ["date", "instrument", "ttm_deducted_net_profit", "total_market_cap"]].copy()
    df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
    if df["date"].isna().any():
        raise ValueError("earnings_to_price_deducted 的 date 存在无法解析的日期或缺失值。")
    if df["instrument"].isna().any():
        raise ValueError("earnings_to_price_deducted 的 instrument 不允许缺失。")
    ...
    if duplicated.any():
        ...
        raise ValueError("earnings_to_price_deducted 输入存在重复 date + instrument：" + str(examples))
    ...
    if show_progress:
        print(
            "\r[earnings_to_price_deducted] 计算目标截面 | "
            + str(len(target_index)) + " 个日期 | 已耗时 0.0s",
            end="", flush=True,
        )
    try:
        numerator = pd.to_numeric(df["ttm_deducted_net_profit"], errors="coerce")
        denominator = pd.to_numeric(df["total_market_cap"], errors="coerce")
        valid_denominator = denominator > 0 if "positive" == "positive" else denominator != 0
        values = numerator.where(valid_denominator) / denominator.where(valid_denominator)
        result = pd.DataFrame({
            "date": df["date"].to_numpy(),
            "instrument": df["instrument"].to_numpy(),
            "earnings_to_price_deducted": values.to_numpy(dtype=float),
        })
        ...
        if show_progress:
            ...
            print(
                "\r[earnings_to_price_deducted] 计算完成 | "
...
FACTOR = {
    "name": "earnings_to_price_deducted",
    "func": calc_earnings_to_price_deducted,
    "factor_type": "base",
    "category": "value",
    "direction": 1,
    "description": "扣非净利润相对市值的估值因子，数值越高代表扣非盈利相对价格越高。",
    "formula": "factor_t = ttm_deducted_net_profit_t / total_market_cap_t。",
    "input_schema": {
        "required": {
            ...
            "ttm_deducted_net_profit": {"dtype": "float", "frequency": "financial", "meaning": "按点时口径可得的扣非净利润（TTM）。"},
            "total_market_cap": {"dtype": "float", "frequency": "daily", "meaning": "目标日总市值，必须为正。"},
        },
        "conditional": {},
    },
    "parameters": {
    ...
    "output_schema": {
        ...
        "earnings_to_price_deducted": {"dtype": "float64", "meaning": "扣非净利润相对市值的估值因子，数值越高代表扣非盈利相对价格越高。"},
    },
    ...
    "tags": ["value", "financial", "base_factor"],
    "status": "research",
    "version": "1.0.0",
}
...
以 TTM 扣非净利润除以目标日总市值，衡量剔除非经常性损益后的盈利估值。仅在总市值为正时有效。
